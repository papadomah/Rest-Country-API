...
//to implement the search countries by search impute
  const searchCountries = async () => {
    ...
    if (searchValue.trim()) {
      try {
        let url = `https://restcountries.com/v2/name/${searchValue}`;
  
        if (selectValue !== 'All') {
          const response = await fetch(`https://restcountries.com/v2/region/${selectValue}`);
          const regionData = await response.json();
  
          if (regionData.status === 404) {
            setCountries([]);
            return;
          }
  
          const filteredCountries = regionData.filter(country => country.name.toLowerCase().includes(searchValue.toLowerCase()));
